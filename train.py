...
def parse_args()->object: 
    ...
    parser.add_argument("--use-dense-rewards", default=True, type=boolean_string, help="Whether or not to use intermediate rewards")
...
if args.default: 
    ...
    self_play=False
    model_path=None # if training with opponent, add path here
    logwandb = True
        ...
        if duration_penalty: 
            ...
            if use_symmetries:
                ...
                if use_dense_rewards:   
                    # can similarly set up CustomOpponentEnv_V5
                    env = CustomOpponentEnv_V4()
                    version = "v4"
                    # Set up opponent for V4 environment, comment out for random opponent
                    if model_path:
                        #opponent = PPO.load(model_path, env=env, custom_objects={'learning_rate': 0.0, "clip_range": 0.0, "lr_schedule":0.0})
                        opponent = MaskablePPO.load(model_path, env=env, custom_objects={'learning_rate': 0.0, "clip_range": 0.0, "lr_schedule":0.0})
                        opponent.set_env(env=env)
